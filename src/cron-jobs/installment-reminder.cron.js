...
const installmentReminder = async () => {
  cron.schedule(
    '00 00 09 * * *', // Every day on 09:00:00
    async () => {
      ...
      try {
        const installments = await StudentCourseInstallment.db
          .getCollection('studentcourseinstallments')
          .aggregate([
            {
              $match: {
                installments: {
                  $elemMatch: {
                    $or: [
                      { installmentDate: { $eq: toDate } },
                      { installmentDate: { $eq: tomorrowDate } },
                    ],
                    receiptId: null,
                  },
                },
            ...
            },
            {
              $match: {
                $or: [
                  { 'installments.installmentDate': { $eq: toDate } },
                  { 'installments.installmentDate': { $eq: tomorrowDate } },
                ],
                receiptId: null,
              },
            },
        ...
        for (let installment of installments) {
          ...
          const installmentMessage = `You Have installment to Pay of INR ${installment.installmentAmount
            .toString()
            .toFixed(2)}/- of course ${installment.courseName} of institute ${
            installment.branchName
          }(${(installment, categoryName)}) on ${installment.installmentDate
            .split('-')
            .reverse()
            .join('-')}`;
...
module.exports = installmentReminder;
