...
const constructStaffStudentHistory = async (branches, branchId) => {
  ...
  branches.forEach((curBranchHistory) => {
    if (curBranchHistory.branch.toString() === branchId.toString()) {
      ...
      curBranchHistory.courseTypes.forEach((curCourseTypeHistory) => {
        ...
        if (thisCourseType.status === '1') {
          ...
          curCourseTypeHistory.courses.forEach((curCourseHistory) => {
            const thisCourse = course.find((curCourse) => curCourseHistory.course == curCourse._id);
            let courseObj = {};
...
const getStaffStudentHistory = async (req, res) => {
  try {
    const branch = await Branch.findById(req.body.branch, { _id: 1 });
    ...
    const studentHistory = await constructStaffStudentHistory(history.branches, branch._id);
